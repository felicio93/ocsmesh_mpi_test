#!/bin/bash
# =============================================================================
# SLURM job script — Smoke-test Config D (dedicated, 8h)
# =============================================================================
# Config D: "full recipe minus topofunc"
#   flags: --skip-topofunc
#
# Cost ladder position: C + global contour/channel
#   Adds global add_contour (0 m + -200 m) and add_channel on top of Config C.
#   These are the O(tiles x contour-segments) bottleneck of the exact method
#   and still run serially on rank 0 via _apply_features (the TODO(mpi) in
#   collector.py). topo_func_constraint is still excluded.
#
# What this isolates:
#   The cost of global contour/channel (the still-un-parallelized
#   _apply_features stage on rank 0). Config D vs Config C shows exactly
#   how much contour/channel adds, which is the next major MPI target.
#
# Expected walltime (15 tiles, 80 cores):
#   mpi       ~2-3h   (contour/channel serial on rank 0 is the bottleneck)
#   parallel  ~3-4h
#   serial_mp ~3-4h
#   total     ~6-8h — tight; use windfall partition if it overruns
#
# NOTE: This is the most expensive config. If it overruns 8h on hercules,
# resubmit on the windfall partition (no walltime limit, lower priority):
#   SBATCH --partition=windfall
#   SBATCH --time=24:00:00
#
# Submit:
#   sbatch slurm_smoke_config_D.sh
#   N_CUDEM=14 sbatch slurm_smoke_config_D.sh
#   MODES="mpi" sbatch slurm_smoke_config_D.sh   # MPI only first
# =============================================================================

#SBATCH --job-name=ocsmesh_smoke_D
...
#SBATCH --output=logs/smoke_D_%j.out
#SBATCH --error=logs/smoke_D_%j.err
...
RESULTS_ROOT="${PROJ}/results/smoke_D_${SLURM_JOB_ID}"
NPROCS=79

# ── Config ────────────────────────────────────────────────────────────────────
CFG="D"
CFG_FLAGS="--skip-topofunc"
CFG_DESC="full recipe minus topofunc (adds global contour/channel + boxes)"
N_CUDEM="${N_CUDEM:-14}"
...
echo " NOTE: This is the most expensive config. If it overruns 8h,"
echo "       resubmit on windfall partition with --time=24:00:00."
echo "================================================================="
...
# ── Step 2: Run all modes for Config D ────────────────────────────────────────
