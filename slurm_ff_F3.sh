#!/bin/bash
# slurm_ff_F3.sh — add_contour only (size=1km)
#SBATCH --job-name=ff_F3
...
#SBATCH --output=logs/ff_F3_%j.out
#SBATCH --error=logs/ff_F3_%j.err
...
OUT="${PROJ}/results/config_ffat/isolation/F3_contour"
...
# F3 gets 6h — add_contour at size=1km is more expensive than size=2km
echo "=== Config F3: add_contour only (size=1km, 45 ranks) ==="
