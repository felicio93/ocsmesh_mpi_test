#!/bin/bash
# =============================================================================
# SLURM job script — Smoke-test Config B (dedicated, 8h)
# =============================================================================
# Config B: "no constraint, 2 refinements per tile"
#   flags: --all-fast-refinements
#
# Cost ladder position: A + more per-tile refinement work
#   Both add_subtidal_flow_limiter AND add_constant_value applied to EVERY
#   CUDEM tile (bypasses the modulo scheme). All constraints, box refinements,
#   and global contour/channel are forced off by --all-fast-refinements.
#
# What this isolates:
#   The effect of doubling per-tile refinement work (14+14 tasks instead of
#   3+3). Compared to Config A this shows how Pool IPC overhead scales with
#   task count, and whether MPI meshdata dispatch time changes when rank 0
#   spends more time on refinements before dispatch.
#
# Expected walltime (15 tiles, 80 cores):
#   mpi       ~20 min
#   parallel  ~80 min  (14+14 refinement tasks + GEBCO bottleneck)
#   serial_mp ~50 min
#   total     ~3h — well within 8h
#
# Submit:
#   sbatch slurm_smoke_config_B.sh
#   N_CUDEM=14 sbatch slurm_smoke_config_B.sh
#   MODES="mpi parallel" sbatch slurm_smoke_config_B.sh
# =============================================================================

#SBATCH --job-name=ocsmesh_smoke_B
...
#SBATCH --output=logs/smoke_B_%j.out
#SBATCH --error=logs/smoke_B_%j.err
...
RESULTS_ROOT="${PROJ}/results/smoke_B_${SLURM_JOB_ID}"
NPROCS=79

# ── Config ────────────────────────────────────────────────────────────────────
CFG="B"
CFG_FLAGS="--all-fast-refinements"
CFG_DESC="no constraint, 2 refinements/tile (flow+const on every tile)"
N_CUDEM="${N_CUDEM:-14}"
...
# ── Step 2: Run all modes for Config B ────────────────────────────────────────
