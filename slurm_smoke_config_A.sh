#!/bin/bash
# =============================================================================
# SLURM job script — Smoke-test Config A (dedicated, 8h)
# =============================================================================
# Config A: "no constraint, 1 refinement per tile"
#   flags: --skip-constraints --skip-box-refinements --light-features
#
# Cost ladder position: BASELINE
#   Only add_subtidal_flow_limiter (modulo class 0) and add_constant_value
#   (modulo class 1) run — one class per tile. All constraints, box
#   refinements, and global contour/channel are skipped.
#
# What this isolates:
#   The pure MPI meshdata dispatch speedup with minimal serial overhead on
#   rank 0. This is the cleanest MPI speedup measurement.
#
# Expected walltime (15 tiles, 80 cores):
#   mpi       ~15 min
#   parallel  ~50 min  (GEBCO tile dominates at ~37 min)
#   serial_mp ~50 min  (same GEBCO bottleneck)
#   total     ~2h — well within 8h
#
# Submit:
#   sbatch slurm_smoke_config_A.sh
#   N_CUDEM=14 sbatch slurm_smoke_config_A.sh   # override tile count
#   MODES="mpi parallel" sbatch slurm_smoke_config_A.sh
# =============================================================================

#SBATCH --job-name=ocsmesh_smoke_A
...
#SBATCH --output=logs/smoke_A_%j.out
#SBATCH --error=logs/smoke_A_%j.err
...
RESULTS_ROOT="${PROJ}/results/smoke_A_${SLURM_JOB_ID}"
NPROCS=79   # = ntasks_per_node - 1

# ── Config ────────────────────────────────────────────────────────────────────
CFG="A"
CFG_FLAGS="--skip-constraints --skip-box-refinements --light-features"
CFG_DESC="no constraint, 1 refinement/tile (modulo scheme)"
N_CUDEM="${N_CUDEM:-14}"
...
# ── Step 2: Run all modes for Config A ────────────────────────────────────────
